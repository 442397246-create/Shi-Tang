...
function openCreateModal() {
    ...
    openModal('生成结算单', content, function() { createSettlement(); });
...
function previewSettlement() {
    ...
    var orders = db.get('purchase_orders').filter(function(p) {
...
function createSettlement() {
    ...
    var supplier = db.get('suppliers').find(function(s) { return s.id === supplierId; });
    var orders = db.get('purchase_orders').filter(function(p) {
        return p.supplier_id === supplierId && p.date >= periodStart && p.date <= periodEnd && (p.status === 'paid' || p.status === 'completed');
    });
...
function viewSettlement(id) {
    var s = db.get('settlements').find(function(x) { return x.id === id; });
    if (!s) return;
...
function approveSettlement(id) {
    openModal('确认审批', '<p>确定审批通过该结算单吗？</p>', function() {
        db.update('settlements', id, { status: 'approved' });
...
function paySettlement(id) {
    openModal('确认付款', '<p>确定执行付款吗？付款后不可撤销。</p>', function() {
        db.update('settlements', id, { status: 'paid' });
