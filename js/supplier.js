...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (alertEl) alertEl.textContent = utils.getInventoryAlertCount() + utils.getPendingApprovalCount();
});
...
function openAddModal() {
    openModal('新增供应商', getFormHTML(null), function() {
        saveSupplier(null);
    });
}

function editSupplier(id) {
    var supplier = db.get('suppliers').find(function(s) { return s.id === id; });
    if (!supplier) return;
    openModal('编辑供应商', getFormHTML(supplier), function() {
        saveSupplier(id);
    });
}
...
function viewSupplier(id) {
    var s = db.get('suppliers').find(function(x) { return x.id === id; });
    if (!s) return;
    var orders = db.get('purchase_orders').filter(function(p) { return p.supplier_id === id; });
...
function toggleStatus(id) {
    ...
    openModal('确认操作', '<p>确定' + (newStatus === 'disabled' ? '停用' : '启用') + '该供应商吗？</p>', function() {
...
function deleteSupplier(id) {
    ...
    openModal('确认删除', '<p>确定删除供应商 <strong>' + escapeHtml(s.name) + '</strong> 吗？此操作不可恢复。</p>', function() {
